...
export default function Navbar({ user, setUser, isLoggedIn, setIsLoggedIn }) {
	...
	const setLink = () => {
		if (window.location.href === 'http://localhost:5173/signup') {
			...
			window.location.href === 'http://localhost:5173/house' ||
			window.location.href === 'http://localhost:5173/pokecenter' ||
			window.location.href === 'http://localhost:5173/pokedex'
		) {
			setLinkWords('Map');
		...
		<>
			<nav className='Navbar' id='nav_bar'>
				<div className='navLinks'>
					...
					<Link to={linkWordsLink} className='nav_link'>
						{' '}
						{linkWords}{' '}
					</Link>
					{isLoggedIn ? (
						<Link onClick={handleLogOut} className='nav_link'>
							Log Out
						</Link>
					) : null}
				</div>
